...
int main() {
    ...
    for (const auto& entry : trace) {
        if (entry.find("action=Write") != std::string::npos && entry.find("segment=meta") != std::string::npos) found_write = true;
        if (entry.find("action=Read") != std::string::npos && entry.find("segment=meta") != std::string::npos) found_read = true;
    }
