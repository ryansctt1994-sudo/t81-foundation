...
void demo_basics() {
    ...
    auto prod = f1 * f2;
    std::cout << "T81Float: " << std::to_string(f1.to_double()) << " * " << std::to_string(f2.to_double()) << " = " << std::to_string(prod.to_double()) << "\n";
...
void demo_math() {
    ...
    auto frsum = fr1 + fr2;
    std::cout << "T81Fraction: 1/3 + 1/2 = " << frsum.num().to_string() << "/" << frsum.den().to_string() << "\n";
...
void demo_cognitive() {
    ...
    std::cout << "T81Agent: Created agent " << agent.identity() << " with fuel=" << agent.fuel_remaining() << "\n";
...
void demo_data_io() {
    ...
    bytes[0] = 0xDE;
    bytes[1] = 0xAD;
    std::cout << "T81Bytes: First two bytes: " << std::hex << (int)bytes[0] << " " << (int)bytes[1] << std::dec << "\n";
