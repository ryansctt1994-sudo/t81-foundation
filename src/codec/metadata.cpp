...
Result<std::pair<Header, std::vector<uint8_t>>> unwrap_encoded_buffer(std::span<const uint8_t> buffer) {
    if (buffer.size() < 16) {
        return T81Error(T81Symbol::intern("INSUFFICIENT_DATA"), T81String("Buffer too small for header"));
    }
    ...
    if (h.version != 1) {
        return T81Error(T81Symbol::intern("UNSUPPORTED_VERSION"), T81String("Unsupported metadata version"));
    }
