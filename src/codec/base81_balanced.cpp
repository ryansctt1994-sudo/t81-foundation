...
Result<size_t> pack(std::span<const Trit> trits, std::span<int8_t> digits) {
    ...
    if (digits.size() < num_digits) {
        return T81Error(T81Symbol::intern("INSUFFICIENT_BUFFER"), T81String("Output digits buffer too small"));
    }
...
Result<size_t> unpack(std::span<const int8_t> digits, std::span<Trit> trits, size_t trit_count) {
    ...
    if (trits.size() < trit_count) {
        return T81Error(T81Symbol::intern("INSUFFICIENT_BUFFER"), T81String("Output trits buffer too small"));
    }

    for (size_t i = 0; i < num_digits; ++i) {
        Trit block[4];
        if (digits[i] < -40 || digits[i] > 40) {
            return T81Error(T81Symbol::intern("INVALID_DIGIT"), T81String("Balanced Base-81 digit out of range [-40, 40]"));
        }
