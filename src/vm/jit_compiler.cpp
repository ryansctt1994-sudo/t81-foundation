...
public:
    ...
    ExecResult execute(State& state) override {
        ...
        auto intern_option = [&state](bool has_value, ValueTag payload_tag, std::int64_t payload) -> std::int64_t {
        ...
        auto intern_result = [&state](bool is_ok, ValueTag payload_tag, std::int64_t payload) -> std::int64_t {
        ...
        auto intern_enum = [&state](int variant_id, bool has_payload, ValueTag payload_tag, std::int64_t payload) -> std::int64_t {
        ...
        for (const auto& insn : insns_) {
            ...
            switch (insn.opcode) {
                case t81::tisc::Opcode::Add:
                    if (!reg_ok(insn.a) || !reg_ok(insn.b) || !reg_ok(insn.c)) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::Div:
                    if (!reg_ok(insn.a) || !reg_ok(insn.b) || !reg_ok(insn.c)) { stop_trace = true; guard_deopt = true; break; }
                    if (state.registers[insn.c] == 0) { stop_trace = true; guard_deopt = true; break; }
                    state.registers[insn.a] = state.registers[insn.b] / state.registers[insn.c];
                    state.register_tags[insn.a] = ValueTag::Int;
                    break;
                ...
                case t81::tisc::Opcode::Inc:
                    if (!reg_ok(insn.a)) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::Mov:
                    if (!reg_ok(insn.a) || !reg_ok(insn.b)) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::Load:
                    if (!reg_ok(insn.a) || !mem_ok(insn.b)) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::Store:
                    if (!reg_ok(insn.b) || !mem_ok(insn.a)) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::Push: {
                    ...
                    if (!stack.valid() || state.sp <= stack.start) { stop_trace = true; guard_deopt = true; break; }
                    std::size_t new_sp = state.sp - 1;
                    if (!stack.contains(new_sp)) { stop_trace = true; guard_deopt = true; break; }
                    state.sp = new_sp;
                ...
                case t81::tisc::Opcode::Pop: {
                    if (!reg_ok(insn.a)) { stop_trace = true; guard_deopt = true; break; }
                    const auto& stack = state.layout.stack;
                    if (!stack.valid() || state.sp >= stack.limit) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::MakeOptionSome:
                    if (!reg_ok(insn.a) || !reg_ok(insn.b)) { stop_trace = true; guard_deopt = true; break; }
                    state.registers[insn.a] = intern_option(true, state.register_tags[insn.b], state.registers[insn.b]);
                ...
                case t81::tisc::Opcode::MakeResultOk:
                    if (!reg_ok(insn.a) || !reg_ok(insn.b)) { stop_trace = true; guard_deopt = true; break; }
                    state.registers[insn.a] = intern_result(true, state.register_tags[insn.b], state.registers[insn.b]);
                    state.register_tags[insn.a] = ValueTag::ResultHandle;
                    break;
                case t81::tisc::Opcode::MakeResultErr:
                    if (!reg_ok(insn.a) || !reg_ok(insn.b)) { stop_trace = true; guard_deopt = true; break; }
                    state.registers[insn.a] = intern_result(false, state.register_tags[insn.b], state.registers[insn.b]);
                    state.register_tags[insn.a] = ValueTag::ResultHandle;
                    break;
                case t81::tisc::Opcode::OptionIsSome: {
                    if (!reg_ok(insn.a) || !reg_ok(insn.b) || state.register_tags[insn.b] != ValueTag::OptionHandle) {
                        stop_trace = true;
                        guard_deopt = true;
                        break;
                    }
                    auto* opt = option_ptr(state.registers[insn.b]);
                    if (!opt) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::OptionUnwrap: {
                    ...
                    auto* opt = option_ptr(state.registers[insn.b]);
                    if (!opt || !opt->has_value) { stop_trace = true; guard_deopt = true; break; }
                    state.registers[insn.a] = opt->payload;
                    state.register_tags[insn.a] = opt->payload_tag;
                    break;
                }
                case t81::tisc::Opcode::ResultIsOk: {
                    if (!reg_ok(insn.a) || !reg_ok(insn.b) || state.register_tags[insn.b] != ValueTag::ResultHandle) {
                        stop_trace = true;
                        guard_deopt = true;
                        break;
                    }
                    auto* res = result_ptr(state.registers[insn.b]);
                    if (!res) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::ResultUnwrapOk: {
                    ...
                    auto* res = result_ptr(state.registers[insn.b]);
                    if (!res || !res->is_ok) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::MakeEnumVariantPayload:
                    ...
                    state.registers[insn.a] = intern_enum(static_cast<int>(insn.c), true, state.register_tags[insn.b], state.registers[insn.b]);
                    state.register_tags[insn.a] = ValueTag::EnumHandle;
                    break;
                case t81::tisc::Opcode::EnumIsVariant: {
                    if (!reg_ok(insn.a) || !reg_ok(insn.b) || state.register_tags[insn.b] != ValueTag::EnumHandle) {
                        stop_trace = true;
                        guard_deopt = true;
                        break;
                    }
                    auto* val = enum_ptr(state.registers[insn.b]);
                    if (!val) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::EnumUnwrapPayload: {
                    ...
                    auto* val = enum_ptr(state.registers[insn.b]);
                    if (!val || !val->has_payload) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::JumpIfZero:
                    if (!reg_ok(insn.b)) { stop_trace = true; guard_deopt = true; break; }
                ...
                case t81::tisc::Opcode::Call: {
                    ...
                    if (!code_ok(target)) { stop_trace = true; guard_deopt = true; break; }
                    const auto& stack = state.layout.stack;
                    if (!stack.valid() || state.sp <= stack.start) { stop_trace = true; guard_deopt = true; break; }
                    std::size_t new_sp = state.sp - 1;
                    if (!stack.contains(new_sp)) { stop_trace = true; guard_deopt = true; break; }
                    ...
                    state.memory[state.sp] = static_cast<std::int64_t>(state.pc + result.instructions_executed);
                ...
                case t81::tisc::Opcode::Ret: {
                    ...
                    state.sp += 1;
                    if (tag != ValueTag::Int || !code_ok(addr)) { stop_trace = true; guard_deopt = true; break; }
            ...
            if (insn.opcode != t81::tisc::Opcode::Jump &&
                ...
                insn.opcode != t81::tisc::Opcode::Call &&
                insn.opcode != t81::tisc::Opcode::Ret &&
                insn.opcode != t81::tisc::Opcode::Store &&
                insn.opcode != t81::tisc::Opcode::Push &&
                insn.opcode != t81::tisc::Opcode::Cmp &&
                insn.opcode != t81::tisc::Opcode::TMatMul &&
