...
inline std::size_t size_of(const std::vector<int>& shp) {
  ...
  return std::accumulate(shp.begin(), shp.end(), std::size_t{1}, [](std::size_t a, int b) {
...
inline std::vector<std::size_t> strides_of(const std::vector<int>& shp) {
  ...
  for (int i = static_cast<int>(shp.size()) - 2; i >= 0; --i) {
    ...
    s[(std::size_t)i] =
        s[(std::size_t)(i + 1)] * static_cast<std::size_t>(shp[(std::size_t)(i + 1)]);
  }
  return s;
}
...
inline std::vector<int> broadcast_shape(const std::vector<int>& a, const std::vector<int>& b) {
  ...
  for (int i = 0; i < r; ++i) {
    ...
    if (da == db)
      out[(std::size_t)(r - 1 - i)] = da;
    else if (da == 1)
      out[(std::size_t)(r - 1 - i)] = db;
    else if (db == 1)
      out[(std::size_t)(r - 1 - i)] = da;
    else
