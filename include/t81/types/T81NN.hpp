 ...
 */
template <size_t InChannels, size_t OutChannels, size_t KernelSize, size_t Stride = 1,
          size_t Padding = 0>
class Conv2d {
public:
  // Weight: (Out, In, K, K)
  ...
  [[nodiscard]] constexpr auto forward(
      const T81Tensor<TF, 4, Batch, InChannels, H, W>& input) const {
...
struct Functional {
  ...
  static constexpr auto relu(const TensorType& t) {
    return t81::relu(t);
  }

  template <typename TensorType>
  static constexpr auto gelu(const TensorType& t) {
    return t81::gelu(t);
  }

  template <typename TensorType>
  static constexpr auto softmax(const TensorType& t) {
    return t81::softmax(t);
  }
};
