...
struct T81Error {
  ...
  T81Error(T81Symbol c, T81String m, T81Symbol src = symbols::unk)
      : code(c),
        message(std::move(m)),
        occurred_at(T81Time::now(src)),
  ...
  T81Error(const T81Error& other)
      : code(other.code),
        ...
        occurred_at(T81Time::now(T81Symbol::intern("ERROR_COPY"))),
        fuel_spent(acquire_entropy())  // copying an error costs fresh entropy
        ,
        source(other.source) {}

  [[nodiscard]] T81String explain() const {
    return T81String("[ERROR ") + T81String(code.to_string()) + T81String(" at ") +
           occurred_at.narrate() + T81String("] ") + message + T81String(" (source: ") +
           T81String(source.to_string()) + T81String(")");
  }
};
...
public:
  ...
  static constexpr T81Result success(T value) noexcept { return T81Result(std::move(value)); }

  static constexpr T81Result failure(T81Symbol code, T81String msg,
                                     T81Symbol src = symbols::unk) noexcept {
...
inline constexpr auto SAFE_DIV = [](T81Int<81> a, T81Int<81> b) -> T81Result<T81Int<81>> {
  if (b.is_zero()) {
    return T81Result<T81Int<81>>::failure(
        errors::DIVISION_BY_ZERO, T81String("Cannot divide by zero — the universe would collapse."),
