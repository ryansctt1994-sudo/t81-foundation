...
public:
  ...
  explicit T81List(size_type n, const E& value = E{}) : data_(n, value) {}

  T81List(std::initializer_list<E> init) : data_(init) {}
  ...
  void push_back(const E& value) { data_.push_back(value); }

  void push_back(E&& value) { data_.push_back(std::move(value)); }
  ...
  T81List& operator+=(T81List&& o) noexcept(std::is_nothrow_move_constructible_v<E>) {
    data_.insert(data_.end(), std::make_move_iterator(o.data_.begin()),
  ...
  [[nodiscard]] std::span<E> span() noexcept { return std::span<E>(data_.data(), data_.size()); }

  [[nodiscard]] std::span<const E> span() const noexcept {
    return std::span<const E>(data_.data(), data_.size());
  }

  [[nodiscard]] const E* data() const noexcept { return data_.data(); }

  [[nodiscard]] E* data() noexcept { return data_.data(); }
};
