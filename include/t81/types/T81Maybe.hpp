...
public:
  ...
  constexpr T81Maybe(std::nullopt_t) noexcept : value_(std::nullopt) {}

  // Explicit "nothing" from nullptr (for legacy/tests)
  constexpr T81Maybe(std::nullptr_t) noexcept : value_(std::nullopt) {}

  // From const value
  constexpr T81Maybe(const T& v) : value_(v) {}
  ...
  // Factory helpers
  static constexpr T81Maybe just(const T& v) { return T81Maybe(v); }

  static constexpr T81Maybe just(T&& v) { return T81Maybe(std::move(v)); }

  // Plain "nothing" (no reason)
  static constexpr T81Maybe nothing() noexcept { return T81Maybe(std::nullopt); }
  ...
  template <typename F>
  [[nodiscard]] auto map(F&& f) const -> T81Maybe<std::invoke_result_t<F, const T&>> {
