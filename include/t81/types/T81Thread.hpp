...
private:
  struct ThreadState {
    ...
    ThreadState(T81Agent a, T81Symbol n)
        : agent(std::move(a)), born_at(T81Time::now(symbols::THREAD_BIRTH)), name(n) {}
...
public:
  ...
  void request_stop() noexcept { state_->agent.observe(symbols::REQUESTED_TO_DIE); }
  ...
  [[nodiscard]] T81Reflection<T81Thread> reflect() const {
    ...
    switch (state_->lifecycle.load()) {
      case State::ALIVE:
        status = symbols::THINKING;
        break;
      case State::STOPPED:
        status = symbols::SLEEPING;
        break;
      case State::JOINED:
        status = T81Symbol::intern("JOINED");
        break;
      case State::DETACHED:
        status = T81Symbol::intern("DETACHED");
        break;
    }
