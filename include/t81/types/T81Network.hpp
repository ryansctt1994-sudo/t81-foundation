...
public:
  ...
  [[nodiscard]] static T81Result<tcp::socket> connect(const T81Endpoint& remote, T81Agent& self) {
    auto fuel = self.consume_entropy();
    if (!fuel) {
      return T81Result<tcp::socket>::failure(
          errors::OUT_OF_ENTROPY, T81String("CANNOT REACH ACROSS THE VOID WITHOUT FUEL"));
    }
    ...
    if (ec) {
      return T81Result<tcp::socket>::failure(
          T81Symbol::intern("INVALID_ADDRESS"),
    ...
    if (ec) {
      return T81Result<tcp::socket>::failure(T81Symbol::intern("CANNOT_CONNECT"),
                                             T81String("FAILED TO REACH ") +
                                                 T81String(remote.to_string().c_str()) +
                                                 T81String(": ") + T81String(ec.message().c_str()));
    }
  ...
  static T81Result<bool> send(tcp::socket& sock, const T81Bytes& message, T81Agent& self) {
    ...
    if (ec) {
      return T81Result<bool>::failure(
          T81Symbol::intern("TRANSMISSION_FAILED"),
  ...
  static T81Result<T81Bytes> receive(tcp::socket& sock, size_t max_bytes, T81Agent& self) {
    auto fuel = self.consume_entropy();
    if (!fuel) {
      return T81Result<T81Bytes>::failure(errors::OUT_OF_ENTROPY,
                                          T81String("LISTENING REQUIRES ATTENTION"));
    }
    ...
    if (ec && ec != asio::error::eof) {
      return T81Result<T81Bytes>::failure(
          T81Symbol::intern("RECEPTION_FAILED"),
