...
public:
  ...
  void shard(const LocalTensor& global) { *local_data_ = global; }

  LocalTensor gather() const { return *local_data_; }

  /**
   * @brief Performs a distributed reduction (sum).
   * In a mock environment, we sum all local data and return.
   */
  Element reduce_sum() const { return t81::reduce_sum(*local_data_); }
