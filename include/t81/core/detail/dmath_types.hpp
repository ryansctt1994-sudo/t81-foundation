...
public:
    ...
    constexpr explicit Fixed(int i) : v(Storage(i)) {
        v <<= F;
    }

    // Basic arithmetic
    constexpr Fixed operator+(const Fixed& other) const {
        return Fixed(v + other.v);
    }
    ...
    static constexpr Fixed from_decimal(std::int64_t int_part, std::uint64_t frac_part, std::uint32_t frac_digits) {
        Storage s(int_part);
        s <<= F;
    ...
    static Fixed from_float(const v1::T81Float<M, E>& f) {
        ...
        if (shift > 0) s <<= static_cast<size_t>(shift);
        else if (shift < 0) s >>= static_cast<size_t>(-shift);
        return Fixed(s);
    }
    ...
    v1::T81Float<M, E> to_float() const {
        ...
        return v1::T81Float<M, E>::from_components(sign, static_cast<std::int64_t>(lead) - static_cast<std::int64_t>(F), mant);
    }
