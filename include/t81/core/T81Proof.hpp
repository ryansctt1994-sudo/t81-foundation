...
public:
    explicit T81Proof(const T81Theorem& th)
        : theorem_name_(th.name)
        , goal_(th.logical_form) {}
    ...
    bool apply_rule(const T81InferenceRule& rule, T81Agent& prover) {
        ...
        if (!rule.validator || rule.validator(steps_.empty() ? nullptr : steps_.back())) {
            ...
            audit_trail_.push_back(T81Reflection<T81Symbol>(rule.name, symbols::PROOF, T81Symbol::intern("STEP")));
            return true;
        }
        return false;
    }
