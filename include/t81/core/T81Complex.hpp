...
public:
  using FloatType = std::conditional_t<MantissaTrits == 18, T81Float<18, 9>, T81Float<27, 9>>;
...
public:
  ...
  constexpr T81Complex(const Float& real, const Float& imag) noexcept : re(real), im(imag) {}
  ...
  static T81Complex one() noexcept { return {Float::from_double(1.0), Float::from_double(0.0)}; }

  static T81Complex i() noexcept { return {Float::from_double(0.0), Float::from_double(1.0)}; }
  ...
  [[nodiscard]] T81Complex operator-() const noexcept { return {-re, -im}; }
  ...
  [[nodiscard]] T81Complex conj() const noexcept { return {re, -im}; }

  [[nodiscard]] Float mag2() const noexcept { return re * re + im * im; }
