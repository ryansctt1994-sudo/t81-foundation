...
public:
  ...
  [[nodiscard]] static std::array<int8_t, TRITS> booth_mul_trits(
      const std::array<int8_t, TRITS>& a, const std::array<int8_t, TRITS>& b) noexcept;
  ...
  [[nodiscard]] static std::pair<T81Limb, T81Limb> mul_wide(const T81Limb& a,
                                                            const T81Limb& b) noexcept;

private:
  [[nodiscard]] static std::pair<T81Limb, T81Limb> mul_wide_canonical(const T81Limb& a,
                                                                      const T81Limb& b) noexcept;
...
inline T81Limb T81Limb::from_trits(const std::array<int8_t, TRITS>& digits) noexcept {
  ...
  T81Limb limb;
  for (int idx = 0; idx < TRYTES; ++idx) {
    int8_t chunk[3] = {normalized[idx * 3 + 0], normalized[idx * 3 + 1], normalized[idx * 3 + 2]};
...
public:
  ...
  [[nodiscard]] static T81Limb54 booth_mul_partial(const T81Limb54& a, const T81Limb54& b,
...
inline constexpr std::pair<T81Limb54, int8_t> T81Limb54::addc(
    const T81Limb54& other) const noexcept {
  T81Limb54 result;
  std::array<int, TRYTES> map_ids{};
  std::array<std::array<int8_t, 3>, TRYTES> sum_vals{};
...
inline T81Limb54 T81Limb54::from_trits(const std::array<int8_t, TRITS>& digits) noexcept {
  ...
  for (int i = 0; i < TRITS; ++i) {
    ...
    if (sum == 2) {
      ...
      normalized[i] = static_cast<int8_t>(sum);
      carry = 0;
    }
  }
  T81Limb54 limb;
...
inline T81Limb54 T81Limb54::booth_mul_partial(const T81Limb54& a, const T81Limb54& b,
                                              int active_trytes) noexcept {
...
inline std::pair<T81Limb, T81Limb> T81Limb::mul_wide_canonical(const T81Limb& a,
                                                               const T81Limb& b) noexcept {
  auto a_trits = a.to_trits();
  auto b_trits = b.to_trits();
  std::array<int, detail::WIDE_TRITS> accum{};
...
public:
  ...
  static T81Limb27 from_trits_window(const std::array<int8_t, T81Limb::TRITS>& digits,
                                     int start_trit) noexcept;
};

inline T81Limb27 T81Limb27::from_trits_window(const std::array<int8_t, T81Limb::TRITS>& digits,
                                              int start_trit) noexcept {
  T81Limb27 limb{};
...
inline std::pair<T81Limb, T81Limb> T81Limb::mul_wide(const T81Limb& a, const T81Limb& b) noexcept {
  auto a_trits = a.to_trits();
  auto b_trits = b.to_trits();
