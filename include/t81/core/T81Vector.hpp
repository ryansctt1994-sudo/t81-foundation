...
class T81Vector {
  ...
  static constexpr Scalar component_to_scalar(const C& c) noexcept {
    ...
    } else if constexpr (requires(const C& x) {
                           { x.to_double() } -> std::convertible_to<double>;
                         }) {
...
public:
  ...
  constexpr T81Vector(Components... comps) noexcept : components_{component_to_scalar(comps)...} {}
  ...
  [[nodiscard]] constexpr const Scalar& operator[](std::size_t i) const noexcept {
  ...
  [[nodiscard]] static constexpr T81Vector one() noexcept { return unit_vector<0>(); }
  ...
  [[nodiscard]] Scalar length() const noexcept {
    if constexpr (requires(const Scalar& s) {
                    { s.to_double() } -> std::convertible_to<double>;
                  }) {
  ...
  [[nodiscard]] std::enable_if_t<M == 3, T81Vector> cross(const T81Vector& o) const noexcept {
    return T81Vector(components_[1] * o[2] - components_[2] * o[1],
  ...
  [[nodiscard]] std::enable_if_t<M == 3, T81Vector> rotated(const T81Quaternion& q) const noexcept {
    ...
    T81Quaternion vq(QScalar::from_double(0.0), qx, qy, qz);
