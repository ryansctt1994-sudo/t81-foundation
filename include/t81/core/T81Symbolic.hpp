...
// Node variants
struct BinaryOp { Op op; ExprPtr lhs; ExprPtr rhs; };
struct UnaryOp  { Op op; ExprPtr operand; };
struct Constant { T81Float<72,9> val; };
struct Variable { T81Symbol sym; };
...
struct Expr : std::enable_shared_from_this<Expr> {
    ...
    static ExprPtr make_const(T81Float<72,9> v) {
        return std::make_shared<Expr>(Constant{v});
    }

    static ExprPtr make_var(T81Symbol s) {
        return std::make_shared<Expr>(Variable{s});
    }
...
inline ExprPtr diff(ExprPtr e, T81Symbol var) {
    struct DiffVisitor {
        ...
        ExprPtr operator()(const Constant&) {
        ...
        ExprPtr operator()(const UnaryOp& u) {
            ...
            switch (u.op) {
                case Op::Neg: return -d_inner;
                case Op::Sin: return cos(u.operand) * d_inner;
                case Op::Cos: return -sin(u.operand) * d_inner;
                case Op::Exp: return exp(u.operand) * d_inner;
                case Op::Log: return d_inner / u.operand;
                default: return Expr::make_const(T81Float<72,9>(0)); // Should not happen
            }
        }
        ...
        ExprPtr operator()(const BinaryOp& b) {
            ...
            switch (b.op) {
                ...
                case Op::Sub: return dl - dr;
                case Op::Mul: return dl * b.rhs + b.lhs * dr;
                case Op::Div:
                    // (u/v)' = (u'v - uv') / v^2
                ...
                default: return Expr::make_const(T81Float<72,9>(0));
            }
        }
    };
    ...
    struct SimpVisitor {
        ...
        ExprPtr operator()(const UnaryOp& u) {
            ...
            if (std::holds_alternative<Constant>(s->node)) {
                ...
                // Other ops would require T81Float math functions which might not be fully exposed or constexpr
            }
        ...
        ExprPtr operator()(const BinaryOp& b) {
            ...
            if (std::holds_alternative<Constant>(l->node) && std::holds_alternative<Constant>(r->node)) {
                ...
                switch (b.op) {
                    case Op::Add: return Expr::make_const(lv + rv);
                    case Op::Sub: return Expr::make_const(lv - rv);
                    case Op::Mul: return Expr::make_const(lv * rv);
                    case Op::Div: return Expr::make_const(lv / rv);
                    // Pow...
                    default: break;
                }
            }
