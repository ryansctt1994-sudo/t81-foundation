...
fs::path fixture_root() {
  return fs::path(__FILE__).parent_path().parent_path() / "fixtures" / "t81lang_std_polynomial";
}
...
int main() {
  ...
  for (const auto& fixture : fixtures) {
    ...
    const fs::path tisc_path =
        fs::temp_directory_path() / ("t81-cli-std-polynomial-" + fixture.stem().string() + ".tisc");

    const int rc = t81::cli::compile(fixture, tisc_path);
    if (rc != 0) {
    ...
    const auto program = t81::tisc::load_program(tisc_path.string());
  ...
  std::cout << "cli std.polynomial fixture test passed!\n";
  return 0;
}
