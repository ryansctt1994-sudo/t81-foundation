...
public:
  static void test_to_std_chunks_capacity() {
    ...
    // We need a negative value that causes a borrow from next, and if next is not there, it pushes
    // back 0 then borrows from it? Wait, if next is not there: if (i + 1 >= chunks.size())
    // chunks.push_back(0); So valid balanced ternary like { -1 } becomes { B-1, -1 }. Then loop
    // continues to i=1. chunks[1] is -1. chunks[1] += B, push_back 0 (now size 3), chunks[2] -= 1.
    ...
    // So {-1} is not a valid input for `to_std_chunks` in the context of `T81BigInt` logic, unless
    // it resolves to positive. e.g. {-1, 1} = -1 + B. Positive. i=0: chunks[0]=-1 < 0.
    // chunks[0]+=B. chunks[1]-=1. chunks is {B-1, 0}. i=1: chunks[1]=0. OK. No push_back.
    ...
    std::cout << "to_std_chunks(10000 * B): size=" << chunks.size()
              << ", capacity=" << chunks.capacity() << "\n";
    ...
    if (chunks.capacity() > chunks.size() + 100) {
      std::cerr << "PATHOLOGY DETECTED: Capacity exploded (" << chunks.capacity() << " vs "
                << chunks.size() << ")\n";
      std::abort();
    } else {
