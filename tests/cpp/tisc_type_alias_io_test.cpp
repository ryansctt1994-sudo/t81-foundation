...
void test_type_alias_io_roundtrip() {
  ...
  [[maybe_unused]] t81::tisc::TypeAliasMetadata box_alias;
  ...
  [[maybe_unused]] t81::tisc::TypeAliasMetadata flag_alias;
  ...
  [[maybe_unused]] t81::tisc::TypeAliasMetadata vertex_alias;
  ...
  [[maybe_unused]] auto temp_file =
      std::filesystem::temp_directory_path() / "tisc_alias_io_test.bin";
  t81::tisc::save_program(program, temp_file.string());

  [[maybe_unused]] auto loaded = t81::tisc::load_program(temp_file.string());
  ...
  auto matches_variants = [](const std::vector<VariantInfo>& lhs,
                             const std::vector<VariantInfo>& rhs) {
    if (lhs.size() != rhs.size()) return false;
    for (size_t i = 0; i < lhs.size(); ++i) {
