...
void test_reflection_non_interference() {
    ...
    for (const auto& event : vm->state().axion_log) {
        if (event.opcode == Opcode::MetaRead) {
            assert(event.verdict.reason.find("MetaRead reflection segment=code addr=5") != std::string::npos);
            found_metaread = true;
        } else if (event.opcode == Opcode::MetaWrite) {
            assert(event.verdict.reason.find("MetaWrite reflection segment=code addr=5") != std::string::npos);
            found_metawrite = true;
        } else if (event.verdict.reason.find("bounds fault segment=unknown addr=9999") != std::string::npos) {
