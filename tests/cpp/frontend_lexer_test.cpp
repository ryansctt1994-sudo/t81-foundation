...
bool test_sequence(const char* source, const std::vector<ExpectedToken>& expected_tokens) {
  ...
  if (tokens.size() != expected_tokens.size() + 1) {
    ...
    for (size_t i = 0; i < tokens.size(); ++i) {
      ...
      std::cerr << "  [" << i << "] type=" << static_cast<int>(tok.type) << " lexeme='"
                << tok.lexeme << "' line=" << tok.line << " col=" << tok.column << "\n";
    }
    return false;
  }
  ...
  for (size_t i = 0; i < expected_tokens.size(); ++i) {
    ...
    if (actual.type != expected.type || actual.lexeme != expected.lexeme ||
      ...
      std::cerr << "  Expected: Type=" << static_cast<int>(expected.type) << ", Lexeme='"
                << expected.lexeme << "', Line=" << expected.line << ", Col=" << expected.column
                << std::endl;
      std::cerr << "  Actual:   Type=" << static_cast<int>(actual.type) << ", Lexeme='"
                << actual.lexeme << "', Line=" << actual.line << ", Col=" << actual.column
                << std::endl;
    }
...
fn main() -> i32 {
  ...
  std::vector<ExpectedToken> expected = {
      {TokenType::Module, "module", 1, 1},   {TokenType::Identifier, "my_mod", 1, 8},
      {TokenType::Semicolon, ";", 1, 14},    {TokenType::Fn, "fn", 3, 1},
      {TokenType::Identifier, "main", 3, 4}, {TokenType::LParen, "(", 3, 8},
      {TokenType::RParen, ")", 3, 9},        {TokenType::Arrow, "->", 3, 11},
      {TokenType::I32, "i32", 3, 14},        {TokenType::LBrace, "{", 3, 18},
      {TokenType::Let, "let", 4, 5},         {TokenType::Identifier, "x", 4, 9},
      {TokenType::Equal, "=", 4, 11},        {TokenType::Integer, "1", 4, 13},
      {TokenType::Semicolon, ";", 4, 14},    {TokenType::Return, "return", 5, 5},
      {TokenType::Identifier, "x", 5, 12},   {TokenType::Semicolon, ";", 5, 13},
