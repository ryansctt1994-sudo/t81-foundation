...
int main() {
  ...
  // Branch taken when zero; ensures flags and PC update match spec intent.
  {
    ...
    // pc0: r2 = 0 (sets zero flag)
    p.insns.push_back({tisc::Opcode::LoadImm, 2, 0, 0});
    // pc1: if r2 == 0 jump to pc3
    p.insns.push_back({tisc::Opcode::JumpIfZero, 3, 2, 0});
  ...
  // Invalid jump target should trap and be logged.
  {
    ...
    if (!expect(vm->state().trace.back().trap.has_value(), "last trace missing trap marker"))
      return 1;
  }
  ...
  {
    [[maybe_unused]] tisc::Program p;
    p.insns.push_back({tisc::Opcode::LoadImm, 2, 1, 0});
    p.insns.push_back({tisc::Opcode::JumpIfNotZero, 3, 2, 0});
  ...
  {
    [[maybe_unused]] tisc::Program p;
    p.insns.push_back({tisc::Opcode::LoadImm, 3, 4, 0});   // target index
    p.insns.push_back({tisc::Opcode::Call, 0, 3, 0});      // call function at r3
