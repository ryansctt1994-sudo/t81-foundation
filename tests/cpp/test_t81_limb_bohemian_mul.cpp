...
T81Limb RandomLimb(std::mt19937_64& rng, std::uniform_int_distribution<int>& dist) {
  ...
  for (int i = 0; i < T81Limb::TRYTES; ++i) limb.set_tryte(i, static_cast<int8_t>(dist(rng)));
  return limb;
}
...
int main() {
  ...
  for (int t = 0; t < TRIALS; ++t) {
    [[maybe_unused]] T81Limb a = RandomLimb(rng, dist);
    [[maybe_unused]] T81Limb b = RandomLimb(rng, dist);

    [[maybe_unused]] T81Limb expected = T81Limb::reference_mul(a, b);
    [[maybe_unused]] T81Limb actual = a * b;
  ...
  std::cout << "bohemian_mul matches operator* on " << TRIALS
            << " random trials — multiplication verified\n";
  return 0;
}
