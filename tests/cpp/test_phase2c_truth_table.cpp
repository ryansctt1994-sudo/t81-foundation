    ...
    for (int8_t a : domain) {
        ...
        for (int8_t b : domain) {
            ...
            // TAnd
            {
                ...
                if (ref != lut_res || ref != swar_res) {
                    std::cerr << "FAIL: TAnd(" << fmt_trit(a) << ", " << fmt_trit(b) << ") -> Ref=" << fmt_trit(ref)
                              << " LUT=" << fmt_trit(lut_res) << " SWAR=" << fmt_trit(swar_res) << std::endl;
                    failures++;
                }
            }
            ...
            // TOr
            {
                ...
                if (ref != lut_res || ref != swar_res) {
                    std::cerr << "FAIL: TOr(" << fmt_trit(a) << ", " << fmt_trit(b) << ") -> Ref=" << fmt_trit(ref)
                              << " LUT=" << fmt_trit(lut_res) << " SWAR=" << fmt_trit(swar_res) << std::endl;
                    failures++;
                }
            }
            ...
            // TXor (Check LUT path matches Ref)
            {
                ...
                if (ref != res) {
                     std::cerr << "FAIL: TXor(" << fmt_trit(a) << ", " << fmt_trit(b) << ") -> Ref=" << fmt_trit(ref)
                              << " Impl=" << fmt_trit(res) << std::endl;
                    failures++;
                }
            }
        }
...
void test_txor_commutativity_check() {
    ...
    if (ab != ba) {
        ...
        if (!found) {
            std::cerr << "FAIL: TXor is commutative for all inputs. This contradicts the spec 'Difference'." << std::endl;
            std::exit(1);
        }
    }
}
