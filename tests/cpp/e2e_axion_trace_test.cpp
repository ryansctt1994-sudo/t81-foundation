...
int main() {
  ...
  [[maybe_unused]] auto program_opt =
      t81::cli::build_program_from_source(std::string(source), "<axion-e2e>");
  if (!program_opt) {
  ...
  for (const auto& entry : vm->state().axion_log) {
    ...
    if (entry.opcode == t81::tisc::Opcode::Nop &&
        reason.find("guard-expr") != std::string_view::npos) {
      saw_match_metadata = true;
    }
    if (entry.opcode == t81::tisc::Opcode::EnumIsVariant &&
        reason.find("variant=Blue") != std::string_view::npos) {
  ...
  if (!saw_match_metadata || !saw_guard_pass || !saw_payload_entry) {
    std::cerr << "Axion trace missing expectation: " << "match-metadata=" << saw_match_metadata
              << " guard-pass=" << saw_guard_pass << " payload-entry=" << saw_payload_entry << '\n';
    return 1;
  }
