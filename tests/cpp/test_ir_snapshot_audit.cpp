...
bool check_opcode_sequence(const std::vector<Instruction>& instructions, const std::vector<Opcode>& expected) {
    auto it = instructions.begin();
    for (const auto& op : expected) {
        it = std::find_if(it, instructions.end(), [&](const Instruction& inst) {
...
void run_test(const std::string& name, const std::string& source, const std::vector<Opcode>& expected_sequence, const std::vector<Opcode>& forbidden_sequence = {}) {
    ...
    // IRGenerator usually requires typed AST for things like method calls, but maybe not for basic ops.
    // Let's try attaching it.
    ...
    if (!forbidden_sequence.empty() && check_opcode_sequence(instructions, forbidden_sequence)) {
        std::cout << "[FAIL] Forbidden opcode sequence found (indicates wrong precedence)." << std::endl;
        passed = false;
    }
    ...
    if (passed) {
        ...
        for (const auto& inst : instructions) {
             // We don't have a to_string for Opcode handy here easily without including extra headers or implementing it.
             // So just print raw int for now or skip.
...
int main() {
    ...
    run_test("Precedence: ~ vs +",
             decls + "let _ = ~a + 1",
             {Opcode::BITNOT, Opcode::ADD},
             {Opcode::ADD, Opcode::BITNOT});

    // 4. Bitwise OR vs Logical AND
    // SKIP: IRGenerator does not support logical && / || yet.
    // run_test("Precedence: | vs &&", ...);
    std::cout << "Skipping test: Precedence: | vs && (Logical operators not implemented in IRGenerator)" << std::endl;
