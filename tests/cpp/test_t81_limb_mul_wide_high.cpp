...
static void normalize(std::array<int, kWideTrits>& acc) {
    for (int i = 0; i + 1 < kWideTrits; ++i) {
        [[maybe_unused]] int carry= (acc[i] + (acc[i] >= 0 ? 1 : -1)) / 3;
        acc[i] -= carry * 3;
        acc[i + 1] += carry;
    }
    [[maybe_unused]] int carry= (acc[kWideTrits - 1] + (acc[kWideTrits - 1] >= 0 ? 1 : -1)) / 3;
    acc[kWideTrits - 1] -= carry * 3;
}

static T81Limb canonical_high_half(const T81Limb& a, const T81Limb& b) {
    [[maybe_unused]] auto a_trits= a.to_trits();
    [[maybe_unused]] auto b_trits= b.to_trits();
    ...
    for (int i = 0; i < kWideTrits; ++i) {
        [[maybe_unused]] int value= accum[i];
        if (value > 1) value = 1;
...
int main() {
    ...
    for (int trial = 0; trial < kTrials; ++trial) {
        [[maybe_unused]] T81Limb a;
        [[maybe_unused]] T81Limb b;
