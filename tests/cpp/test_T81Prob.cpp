...
int main() {
  ...
  if (sat_max != max_val) {
    std::cerr << "WARNING: Max + 1 did not saturate exactly to Max, but didn't wrap. Val: "
              << sat_max.log_odds().to_int64() << "\n";
  ...
  if (sat_min != min_val) {
    std::cerr << "WARNING: Min - 1 did not saturate exactly to Min, but didn't wrap. Val: "
              << sat_min.log_odds().to_int64() << "\n";
  }
  ...
  if (normalized.log_odds().to_int64() != 50) {
    std::cerr << "FAILED: log_softmax_normalize(100, 50) != 50. Got: "
              << normalized.log_odds().to_int64() << "\n";
    return 1;
  }
