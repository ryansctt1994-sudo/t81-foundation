...
void check_success(std::int64_t val) {
    ...
    try {
        ...
        if (static_cast<std::int64_t>(result) != val) {
             std::cerr << "FAILED: Value mismatch for val=" << val
                       << " -> " << typeid(T).name() << " got " << static_cast<std::int64_t>(result) << "\n";
             std::exit(1);
        }
...
int main() {
    ...
    check_success<std::uint64_t>(static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max()));
