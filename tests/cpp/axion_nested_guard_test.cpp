...
int main() {
  ...
  [[maybe_unused]] auto program_opt =
      t81::cli::build_program_from_source(std::string(source), "<axion-nested-guard>");
  if (!program_opt) {
  ...
  if (program.match_metadata_text.find("(guards true)") == std::string::npos ||
    ...
    std::cerr << "Match metadata missing guard annotations: " << program.match_metadata_text
              << std::endl;
    return 1;
  }
  if (program.match_metadata_text.find("guard-expr \"v > 5\"") == std::string::npos) {
    std::cerr << "Match metadata missing guard expression snippet: " << program.match_metadata_text
              << std::endl;
    return 1;
  }
  ...
  [[maybe_unused]] bool saw_payload_log = false;
