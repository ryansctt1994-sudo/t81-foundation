...
fs::path fixture_root() {
  return fs::path(__FILE__).parent_path().parent_path() / "fixtures" / "t81lang_std_runtime";
}
...
int main() {
  ...
  for (const auto& fixture : fixtures) {
    ...
    const fs::path tisc_path =
        fs::temp_directory_path() / ("t81-cli-std-runtime-" + fixture.stem().string() + ".tisc");
  ...
  std::cout << "cli std.runtime fixture test passed!\n";
  return 0;
}
