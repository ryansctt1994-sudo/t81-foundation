...
const std::vector<std::string> KEYWORDS = {
    "module", "type", "const", "export", "fn", "let", "var", "record", "enum", "if", "else", "for",
    "in", "while", "loop", "reflect", "break", "continue", "return", "match", "true", "false",
    "void", "bool", "i32", "i16", "i8", "i2", "BigInt", "Float",
    "Fraction",  // Correct names for types? Based on usage, T81BigInt usually maps to BigInt
                 // keyword or similar?
...
void fuzz_iteration(std::mt19937& gen, int iteration) {
  ...
  try {
    ...
    // We don't want to abort on C++ exceptions if they are expected (e.g. parser errors thrown as
    // exceptions?), but normally the parser uses return values/error flags. If it throws, it might
    // be a bug or intended. For now, let's catch standard exceptions but abort on unknown ones or
    // segfaults (implicit).
  } catch (...) {
