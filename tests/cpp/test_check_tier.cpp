...
int main() {
  ...
  // Case 1: Requirement satisfied
  {
    ...
    if (!expect(v.kind == VerdictKind::Allow, "Tier check failed when requirement met (2 >= 2)")) return 1;

    ctx.current_tier = 3; // Current tier 3
    v = engine.evaluate(ctx);
    if (!expect(v.kind == VerdictKind::Allow, "Tier check failed when requirement exceeded (3 >= 2)")) return 1;
  }
  ...
  // Case 2: Requirement not satisfied
  {
    ...
    Verdict v = engine.evaluate(ctx);
    if (!expect(v.kind == VerdictKind::Deny, "Tier check allowed when requirement not met (2 < 3)")) return 1;
