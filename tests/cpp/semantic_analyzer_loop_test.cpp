...
void expect_semantic_success(const std::string& source, const char* label = "<success fixture>") {
  ...
  if (analyzer.had_error()) {
    std::cerr << "Semantic analyzer reported error for success fixture (" << label << ")"
              << std::endl;
    std::exit(1);
  }
}
...
struct AnalyzerFixture {
  ...
  explicit AnalyzerFixture(std::vector<std::unique_ptr<Stmt>> stmts)
      : statements(std::move(stmts)), analyzer(statements) {
    analyzer.analyze();
    if (analyzer.had_error()) {
...
int main() {
...

  {
    ...
    if (guard_meta.bound_kind != LoopStmt::BoundKind::Guarded || !guard_meta.guard_present ||
