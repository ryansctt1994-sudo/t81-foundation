...
int main() {
  ...
  // Failure case
  [[maybe_unused]] T81Symbol error_code = T81Symbol::intern("TEST_ERROR");
  T81Result<T81Int<27>> failure =
      T81Result<T81Int<27>>::failure(error_code, T81String("Test error message"));
  ...
  [[maybe_unused]] auto doubled =
      success.map([](const T81Int<27>& x) { return x * T81Int<27>(2); });
  ...
  auto chained = success.and_then([](const T81Int<27>& x) {
    ...
    return T81Result<T81Int<27>>::failure(T81Symbol::intern("NEGATIVE"),
                                          T81String("Negative value"));
  });
