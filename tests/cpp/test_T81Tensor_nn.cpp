...
void test_layer_norm() {
    ...
    input(0,0) = TF(1.0); input(0,1) = TF(2.0); input(0,2) = TF(3.0);
    input(1,0) = TF(4.0); input(1,1) = TF(5.0); input(1,2) = TF(6.0);
...
void test_embedding() {
    ...
    weights(0,0)=TF(0.1); weights(0,1)=TF(0.2);
    weights(1,0)=TF(0.3); weights(1,1)=TF(0.4);
    weights(2,0)=TF(0.5); weights(2,1)=TF(0.6);
    weights(3,0)=TF(0.7); weights(3,1)=TF(0.8);

    // Indices: 2x2
    // [[1, 2], [3, 0]]
    T81Tensor<TI, 2, 2, 2> indices;
    indices(0,0)=TI(1); indices(0,1)=TI(2);
    indices(1,0)=TI(3); indices(1,1)=TI(0);
