...
int main() {
  ...
  auto run_test_loop = [&](int iterations, std::uniform_int_distribution<std::int64_t>& dist,
    ...
    for (int i = 0; i < iterations; ++i) {
      ...
      if (!expect(T81BigInt::sub(T81BigInt::add(a, b), b) == a, "add/sub inverse law failed"))
        return false;
      ...
      if (!expect(T81BigInt::mul(a, T81BigInt::one()) == a, "multiplicative identity failed"))
        return false;
      ...
      if (!T81BigInt::is_zero(b)) {
        ...
        if (!T81BigInt::is_zero(g)) {
          if (!expect(T81BigInt::is_zero(T81BigInt::mod(a, g)), "gcd divisibility failed for lhs"))
            return false;
          if (!expect(T81BigInt::is_zero(T81BigInt::mod(b, g)), "gcd divisibility failed for rhs"))
            return false;
        }
      }
    }
  ...
  std::uniform_int_distribution<std::int64_t> dist_large(std::numeric_limits<std::int64_t>::min(),
