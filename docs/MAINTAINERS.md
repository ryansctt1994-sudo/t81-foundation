# Maintainers

> **Source of Truth:** This document lists the **active maintainers** and their responsibilities.

**Last Updated:** February 10, 2026

## 1. Active Maintainers

| User | Role | Focus Areas |
| :--- | :--- | :--- |
| **@t81dev** | **Project Lead** | Architecture, Governance, Core Runtime, Spec |

## 2. Emeritus Maintainers

*(None yet)*

## 3. Becoming a Maintainer

Maintainership is earned through consistent, high-quality contributions and community engagement.

1.  **Contribute:** Submit PRs, review code, help others.
2.  **Nominate:** Existing maintainers can nominate new maintainers.
3.  **Vote:** Maintainers vote on the nomination.

See [GOVERNANCE.md](GOVERNANCE.md) for more details.
