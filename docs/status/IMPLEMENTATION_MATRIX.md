...
| **TISC ISA** | `spec/tisc-spec.md` (Frozen) | **Implemented** | High | Low | Full opcode coverage in `src/tisc` and `src/vm`. |
...
| **Cognitive Tiers** | `spec/cognitive-tiers.md` (Draft) | **Stubbed** | None | High | `src/tiers` contains only documentation placeholders. |
...
3.  **Cleanup**: Remove or properly mark stubbed implementations in `src/tiers` to avoid confusion.
