...
| **System** | | | | | | |
| `Option` | YES | YES | YES | YES (via payload) | YES | Stable |
| `Result` | YES | YES | YES | YES (via payload) | YES | Stable |
...
3.  **Complex Number Persistence Gap:** `T81Complex` is supported in the VM (via `MAKE_COMPLEX`), but lacks binary pool serialization support in `binary_io.cpp`, meaning complex values cannot be persisted in the program binary constants.
4.  **Determinism Testing Gap:** While compiler determinism is tested (`test_ast_ir_compile_repeat_hash_gate`), there are no explicit runtime determinism fixtures for the Collections, `T81Fixed`, or `T81Complex` types.

## Determinism Classification

### Deterministic & Verified
*   **Primitives:** `i2`, `i8`, `i16`, `i32`, `T81BigInt`, `T81Float`.
...
*   **Primitives:** `T81Fraction`, `T81Uint`, `T81Fixed` (aliased to Int).
...
### Surface-Exposed but VM-Incomplete (Polyfills)
*   **Collections:** `T81List`, `T81Map`, `T81Set`, `T81Tree`, `T81Graph`.
*   **Status:** Exposed via handle API but implemented as `Vector` overlays. Canonical serialization is missing (insertion-order dependent).
