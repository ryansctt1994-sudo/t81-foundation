...
- `std.collections`: staged graph helpers `graph_edge_count`, `graph_has_edge`, `graph_add_edge`, and `graph_remove_edge` are now implemented deterministically over flat edge encodings (`[from0, to0, ...]`, odd tails ignored; `graph_add_edge` is idempotent and `graph_remove_edge` removes all matching edges while preserving survivor order).
