...
- `std.sys` / `std.async` / `std.agent`: `exit`, `time`, `entropy`, `yield`, `sleep`, and `self_reflect` are now wired as deterministic frontend/runtime aliases (`exit -> TRAP`, `time -> 0.0`, `entropy -> 0`, `yield/sleep -> no-op`, `self_reflect -> META_REFLECT`) with conformance, IR, and CLI-check coverage.
