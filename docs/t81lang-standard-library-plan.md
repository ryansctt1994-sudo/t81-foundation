...
- Generic user-defined functions: unresolved generic return parameters now fail deterministically when inference cannot bind them (for example `fn none_of[T]() -> Option[T] { return None; }` called as `none_of()`).
