...
- `std.math`: `sin`, `cos`, `tan`, `sqrt`, `exp`, `log`, and `pow` are wired as deterministic frontend/runtime aliases; remaining documented transcendental aliases (`asin`, `acos`, `atan`, `sinh`, `cosh`, `tanh`) currently fail with deterministic "not implemented yet" diagnostics until scalar VM opcode/runtime support is added.
...
- `std.sys` / `std.async` / `std.agent`: `exit`, `time`, `yield`, `sleep`, and `self_reflect` are now wired as deterministic frontend/runtime aliases (`exit -> TRAP`, `time -> 0.0`, `yield/sleep -> no-op`, `self_reflect -> META_REFLECT`) with conformance, IR, and CLI-check coverage.
- `std.tensor`: `load`, `from_list`, `matmul`, and `vec_add` aliases are implemented and tested.
- `std.math` / `std.tensor` / `std.sys` / `std.async` / `std.agent`: wrapper module files now expose the directly wrappable aliases via `src/lang/std/{math,tensor,sys,async,agent}.t81` (`std.tensor.load` remains direct-use because it currently requires a string literal argument at call sites).
