...
- `std.collections`: roadmap module entrypoints `list`, `map`, `set`, `tree`, and `graph` are exposed; `list` now returns a deterministic empty `Vector[T81String]` value, while `map`/`set`/`tree`/`graph` remain stable placeholder handles for staged runtime expansion.
