...
- Generic user-defined functions: call-site explicit type arguments are now supported (`fn id[T](x: T) -> T`, `id[i32](7)`), with deterministic semantic diagnostics for explicit-type-arity and argument-type mismatches plus parser/semantic/conformance coverage.
