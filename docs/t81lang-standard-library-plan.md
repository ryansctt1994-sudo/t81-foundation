...
- `std.sys` / `std.async` / `std.agent`: `exit`, `time`, `entropy`, `proof`, `yield`, `sleep`, `thread`, `promise`, and `self_reflect` are now wired as deterministic frontend/runtime aliases (`exit -> TRAP`, `time -> 0.0`, `entropy -> 0`, `proof/thread/promise -> stable symbolic tokens`, `yield/sleep -> no-op`, `self_reflect -> META_REFLECT`) with conformance, IR, and CLI-check coverage.
- `std.sys` / `std.io` / `std.async`: fixture-driven CLI golden coverage now validates deterministic runtime output for symbolic-token aliases (`proof`, `stream`, `net`, `thread`, `promise`) plus deterministic baseline behavior for `time`, `entropy`, `yield`, and `sleep`.
