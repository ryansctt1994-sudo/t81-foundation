...
- `std.sys` / `std.async` / `std.agent`: `exit`, `time`, `entropy`, `proof`, `yield`, `sleep`, `thread`, `promise`, and `self_reflect` are now wired as deterministic frontend/runtime aliases (`exit -> TRAP`, `time -> 0.0`, `entropy -> 0`, `proof/thread/promise -> stable placeholder handles`, `yield/sleep -> no-op`, `self_reflect -> META_REFLECT`) with conformance, IR, and CLI-check coverage.
...
- `std.collections`: roadmap module entrypoints `list`, `map`, `set`, `tree`, and `graph` are exposed; `list`, `map`, and `set` now return deterministic empty `Vector[T81String]` values, while `tree`/`graph` remain stable placeholder handles for staged runtime expansion.
