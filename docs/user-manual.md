...
```
This exact pipeline is mandated by `AGENTS.md` and reiterated in `README.md`/`docs/onboarding.md` — it must run before publishing changes or citing benchmark numbers.
...
- Use `docs/guides/cli-user-manual.md` and `docs/guides/cli-toolkit.md` for every flag, trace pattern, and automation recipe.
...
- Inspect: `t81 weights info <model.t81w>` reports tensor counts, sparsity, limb layout, and canonical hashes. See `docs/guides/weights-integration.md` for integrating these artifacts into HanoiVM programs (e.g., `weights.load("model.t81w")`).
...
- `benchmarks/benchmark_runner` generates `docs/benchmarks.md`, which is the canonical report for throughput/latency/density comparisons.
- Capture Axion traces with `scripts/capture-axion-trace.sh` to produce `axion_policy_runner.log` and `vm_bounds_trace.log` for auditors; pair these logs with findings from `docs/guides/axion-trace.md` and `docs/guides/axion-policy-manual.md`.

## Architecture & Specification Map

### Logical Layers
- `include/t81/` (`t81::v1`) holds public, header-only data types and APIs.
- `src/` hosts implementations grouped into libraries (`t81_core`, `t81_io`, `t81_tisc`, `t81_frontend`, `t81_vm`) described in `ARCHITECTURE.md`.
...
### Directory Roles (per README + docs/onboarding)
| Path | Role |
| --- | --- |
...
- `./build/t81 --help` lists subcommands such as `compile`, `run`, `repl`, `benchmark`, `weights import`, and `weights quantize` (see `docs/guides/cli-user-manual.md`).
- Use advanced switches (`--trace-guards`, `--emit-axion-log`, `--profile`) to populate Axion trace profiles from `tools/axion-profiler` and match the traces in `docs/guides/runtime-observability-manual.md`.
...
- Document architectural decisions in `ARCHITECTURE.md`, `DESIGN.md`, or `docs/arch` as appropriate; update `docs/tools`, `docs/benchmarks`, or `docs/notes.md` when introducing new observable behavior.
- Report status/issues via `docs/system-status.md`, `CLAUDE.md`, and `ANALYSIS.md` before claiming readiness.
...
- When Axion policies trap, use `t81 repl` → `:trace` to expose the missing `verdict.reason` string referenced by `docs/guides/axion-policy-manual.md`.
- Keep metrics deterministic by always pairing `weights.load` demos with recorded Axion logs and canonical `docs/benchmarks.md` tables.

### Additional Resources
- Guides: `docs/guides/cli-user-manual.md`, `docs/guides/weights-integration.md`, `docs/guides/axion-trace.md`, `docs/guides/runtime-observability-manual.md`, `docs/guides/public-api-overview.md`.
...
- Onboarding & AI agent playbooks: `docs/onboarding.md`, `docs/ai-quickstart.md`.
- Release & governance: `docs/release.md`, `docs/system-status.md`, `spec/rfcs/` rubric, `AGENTS.md`.
