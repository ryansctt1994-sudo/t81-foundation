...
The following conditions are assumed to be true for the system to maintain its determinism guarantees:
1.  **Semantic Invariance:** The host CPU's floating-point or integer behavior does *not* leak into the T81 runtime. All calculations are performed using the `T81Int` or `T729Tensor` libraries.
