#!/usr/bin/env python3
"""Disallow new includes of legacy v1 numeric implementation headers.

Policy:
- Public/canonical numeric path should converge on `t81/bigint.hpp` and
  `t81/fraction.hpp` with compatibility wrappers as needed.
- Legacy implementation headers (`t81/core/T81BigInt.hpp`,
  `t81/core/T81Fraction.hpp`) are migration-only and allowlisted.
"""
...
FORBIDDEN = {
    "t81/core/T81BigInt.hpp",
    "t81/core/T81Fraction.hpp",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Disallow new includes of legacy v1 numeric implementation headers."
    )
    parser.add_argument(
        "--root",
        type=Path,
...
def main() -> int:
    ...
    if violations:
        print("legacy v1 numeric include policy violation(s):")
    ...
    print("legacy v1 numeric include policy: ok")
    return 0
