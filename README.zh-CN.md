...
T81 是一个主权计算栈，旨在消除浮点非确定性并实现完全可审计的执行。通过利用**平衡三进制逻辑**和**81进制数据类型**，T81保证了所有支持架构（x86/ARM，macOS/Linux）上的**比特精确可复现性**。它具有**T81VM**、**Axion安全引擎**以及用于从简单符号逻辑扩展到分布式无限形式的递归层级系统。

> 💡 **为什么重要：** 在AI安全、金融建模和密码学中，“大部分正确”是不够的。T81提供了数学上的确定性，即您的代码在任何地方、任何时间都以完全相同的方式执行。
...
| 特性 | 状态 | 描述 |
| :--- | :--- | :--- |
| **确定性执行** | ✨ 稳定 | 通过 `dmath` 和自定义FP在 x86/ARM/Apple Silicon 上实现比特精确结果。 |
| **三进制原生类型** | ✨ 稳定 | 81进制平衡三进制整数和浮点数（无符号位，进位减少）。 |
| **T81VM 和 TISC** | ✨ 稳定 | 具有确定性解释器和Trace-JIT的81寄存器VM。 |
| **Axion 引擎** | ✨ 稳定 | 运行时策略、安全、伦理和优化引擎，带有审计追踪。 |
| **模型工具** | ✨ 稳定 | 导入/检查 SafeTensors, GGUF, T81W; 量化支持。 |
| **复现性关卡** | ✨ 稳定 | CI强制执行 `t81lang_repro_gate.py` 确保 100% 确定性。 |
